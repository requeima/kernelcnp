...
class LatentNeuralProcess(nn.Module):
    ...
    def forward(self, x_context, y_context, x_target, num_samples):
        ...
        for i in range(num_samples):
            ...
            means.append(z[..., :1])
            noise_scales.append(self.add_noise(torch.zeros(size=(z.shape[0], z.shape[1], z.shape[0]),
                                               z[..., 1:]))
            
        means = torch.stack(means, dim=0)
        noise_scales = torch.stack(noises, dim=0)
    ...
    def loss(self, x_context, y_context, x_target, y_target, num_samples):
        ...
        means, noise_scales = self.forward(x_context,
                                           y_context,
                                           x_target,
                                           num_samples)
        
        # Normal with batch shape (S, B, N, D), event shape ()
        normal = torch.distributions.Normal(loc=means, scales=noise_scales)
        
        # Normal with batch shape (S), event shape (B, N, D)
        normal = torch.distributions.Independent(normal, 3)
        ...
        logprob = mixture.log_prob(y_target) / (B * N * D)
