...
class StandardMeanTEDecoder(StandardDecoder):
    ...
    def forward(self, r_context_means, x_context, y_context, x_target):
        
        r, context_means = r_context_means
        ...
        # Concatenate latents with inputs and pass through decoder.
        # Shape: (batch, num_targets, input_dim + latent_dim).
        z = torch.cat([x_target - context_means, r], -1)
        z = self.post_pooling_fn(z)
...
# =============================================================================
# Fully Connected Fully Equivariant Decoder
# =============================================================================


class FullyConnectedTRREDecoder(nn.Module):
    ...
    def forward(self, r, x_ctx, y_ctx, x_trg):
        ...
        diff = x_ctx - x_trg
        dist = torch.sum(diff ** 2, dim=3)[:, :, :, None]
        
        y_ctx = y_ctx[:, :, None, :]
        y_ctx = y_ctx.repeat(1, 1, dist.shape[2], 1)
        
        r = r[:, :, None, :].repeat(1, 1, dist.shape[2], 1)
        
        ctx = torch.cat([dist, r, y_ctx], dim=-1)
...
class StandardFullyConnectedTEDecoder(FullyConnectedTEDecoder):
    ...
    def __init__(self,
        ...
        # Input dimension of encoder (Din + R)
        element_input_dim = 1 + output_dim + rep_dim
